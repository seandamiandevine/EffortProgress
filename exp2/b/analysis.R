...
# LOAD AND CLEAN ####
setwd("~/Desktop/Ongoing/EffortProg/Exp2/b/")
...
descpre <- 
  d  %>% 
  mutate(age = ifelse(as.numeric(age) > 120, 
                      ifelse(startsWith(age, '19'), 2021-as.numeric(age), NA), 
                      as.numeric(age))) %>% 
  summarise(n = length(unique(id)), 
            mage = mean(age, na.rm = T),
            sdage = sd(age, na.rm = T), 
            msex = mean(ifelse(tolower(gender) %in% c('male', 'female'), 
                               ifelse(tolower(gender) =='male', 1, 0), 
                               NA), na.rm=T))
...
m1.rt <- 
  ...
  lmer(corrt ~ switch * isprog + (1|decisionnum) + (1 + switch|id), data=.)

m1.rt = list(
  lmm = m1.rt, 
  pvalues = car::Anova(m1.rt)
)
...
m1.acc <- 
  ...
  mutate(chosendeck = ifelse(deckchoice == 76, deckl, deckr), 
         ...
         isprog = factor(ifelse(chosendeck %in% 4:6, 1, 0)), 
         efflevel = factor(match(as.numeric(chosendeck) %% 3, 1:6 %% 3))) %>%
  glmer(acc ~ switch * isprog + (1|decisionnum) + (1|id), data=., family = 'binomial')

# Make HTML Table for Switch Costs 
tab_model(m1.rt[[1]], m1.acc,
          ...
          string.ci = '95% CI', file='figures/switchcosttable.html')
...
sc.p2 <- 
  ...
  facet_grid(variable ~ chosendeck, scales = 'free_y', 
             labeller = labeller(
               ...
               variable = c(`meanRT` = "RT", `meanACC` = "% Corr."))) +
...
ggarrange(sc.p2, labels='a',
          ...
          nrow=2) %>% 
  ggexport(filename = 'figures/switchcosts.png', height = 500, width = 850)
...
anova(h1.h3_main_m1, h1.h3_main_m2)
...
for(i in 1:length(pairs)) { 
  ...
  suppressMessages(
    p <- 
      ...
      stat_summary(fun = mean, geom = 'bar', position=position_dodge(width=.9), size=2) + 
...
ggarrange(
  ...
  nrow = 2,
  labels = "a") %>% 
  ggexport(filename = 'figures/H1_H2.png', width = 650)
...
ggsave(grid.arrange(grobs = plots, 
       ...
       filename = 'figures/p_deck.png', height = 7, width = 10)


ggsave(grid.arrange(fp, tbl, nrow = 1, widths= c(1.25, 0.5)), 
         filename = 'figures/forrestplot.png', height = 7, width = 10)

# * FORMATTED REG. RABLE ####
tab_model(h1.h3_main_m2, h2_main_m1, 
