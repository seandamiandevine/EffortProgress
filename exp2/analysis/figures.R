...
# Load --------------------------------------------------------------------
thisexp <- 'b'
setwd('~/Desktop/Ongoing/EffortProg/Exp2/analysis')
...
# tlx ---------------------------------------------------------------------
pdf(paste0(thisexp, 'figures/tlx.pdf'))
...
y  <- tapply(h3$effchoice, as.character(h3$effort_type), mean)
y <- matrix(y[c(1, 3, 2, 5, 4, 6)], 2, 3, 
            dimnames = list(c('01', '10'), c('10%-50%', '10%-90%', '50%-90%')))
se  <- tapply(h3$effchoice, as.character(h3$effort_type), myse)
se  <- matrix(se[c(1, 3, 2, 5, 4, 6)], 2, 3, 
              dimnames = list(c('01', '10'), c('10%-50%', '10%-90%', '50%-90%')))
...
# 2a2b plot ####
pdf('2a2b_hypo.pdf', width=7, height=10)

layout(matrix(1:6, 3, 2))
lettercount = 1

for(thisexp in c('a', 'b')) {
  ...
  thisbar <- 
    barplot(y, 
            ...
            names.arg = c('neither deck yields progress', 'both decks yield progress'), 
            #main='Progress is held constant', 
            legend.text = ifelse(thisexp=='b', T, F), 
            args.legend = list(title='Demand Pairs', 
                               legend=c('10%-50%', 
                                        '10%-90%', 
                                        '50%-90%'))
